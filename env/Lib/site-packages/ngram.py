...
class NGram(set):
    ...
    def __init__(self, items=None, threshold=0.0, warp=1.0, key=None,
        ...
        if key is not None and not isinstance(key, collections.Callable):
    ...
    def items_sharing_ngrams(self, query):
        ...
        for ngram in self.split(query):
            try:
                for match, count in list(self._grams[ngram].items()):
    ...
    def search(self, query, threshold=None):
        ...
        for match, samegrams in list(self.items_sharing_ngrams(query).items()):
